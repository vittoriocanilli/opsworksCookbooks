...
file "#{node['samplecompany-adminold']['ssh_dir_path']}/#{node['samplecompany-adminold']['ssh_key']}" do
  content "#{node['samplecompany-adminold']['ssh_key_content']}"
  mode '0600'
  owner 'root'
  group 'root'
end
...
execute "Retrieve public key from SSH private key" do
  cwd node['samplecompany-adminold']['ssh_dir_path']
  command "ssh-keygen -y -f #{node['samplecompany-adminold']['ssh_key']} > #{node['samplecompany-adminold']['ssh_key']}.pub"
end

# Create the ssh wrapper to be used with GIT
template node['samplecompany-adminold']['github_ssh_wrapper'] do
  ...
  group 'root'
  variables(
    :private_key => "#{node['samplecompany-adminold']['ssh_dir_path']}/#{node['samplecompany-adminold']['ssh_key']}"
   )
end

...
file_location = File.join(node['nginx']['dir'], "sites-available", "samplecompany_apiold")
template file_location do
  ...
  mode    00755
  variables(
    :app_dir => node['samplecompany-apiold']['app_dir'],
    :server_name => node['samplecompany-apiold']['server_name']
   )
end

# Enable the site
nginx_site "samplecompany_apiold" do
  enable  true
end

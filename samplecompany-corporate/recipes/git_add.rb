...
file "#{node['samplecompany-corporate']['ssh_dir_path']}/#{node['samplecompany-corporate']['ssh_key']}" do
  content "#{node['samplecompany-corporate']['ssh_key_content']}"
  mode '0600'
  owner 'root'
  group 'root'
end
...
execute "Retrieve public key from SSH private key" do
  cwd node['samplecompany-corporate']['ssh_dir_path']
  command "ssh-keygen -y -f #{node['samplecompany-corporate']['ssh_key']} > #{node['samplecompany-corporate']['ssh_key']}.pub"
end

# Create the ssh wrapper to be used with GIT
template node['samplecompany-corporate']['github_ssh_wrapper'] do
  ...
  group 'root'
  variables(
    :private_key => "#{node['samplecompany-corporate']['ssh_dir_path']}/#{node['samplecompany-corporate']['ssh_key']}"
   )
end

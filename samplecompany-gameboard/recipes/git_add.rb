...
file "#{node['samplecompany-gameboard']['ssh_dir_path']}/#{node['samplecompany-gameboard']['ssh_key']}" do
  content "#{node['samplecompany-gameboard']['ssh_key_content']}"
  mode '0600'
  owner 'root'
  group 'root'
end
...
execute "Retrieve public key from SSH private key" do
  cwd node['samplecompany-gameboard']['ssh_dir_path']
  command "ssh-keygen -y -f #{node['samplecompany-gameboard']['ssh_key']} > #{node['samplecompany-gameboard']['ssh_key']}.pub"
end

# Create the ssh wrapper to be used with GIT
template node['samplecompany-gameboard']['github_ssh_wrapper'] do
  ...
  group 'root'
  variables(
    :private_key => "#{node['samplecompany-gameboard']['ssh_dir_path']}/#{node['samplecompany-gameboard']['ssh_key']}"
   )
end

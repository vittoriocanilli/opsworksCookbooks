...
file "#{node['samplecompany-logger']['ssh_dir_path']}/#{node['samplecompany-logger']['ssh_key']}" do
  content "#{node['samplecompany-logger']['ssh_key_content']}"
  mode '0600'
  owner 'root'
  group 'root'
end
...
execute "Retrieve public key from SSH private key" do
  cwd node['samplecompany-logger']['ssh_dir_path']
  command "ssh-keygen -y -f #{node['samplecompany-logger']['ssh_key']} > #{node['samplecompany-logger']['ssh_key']}.pub"
end

# Create the ssh wrapper to be used with GIT
template node['samplecompany-logger']['github_ssh_wrapper'] do
  ...
  group 'root'
  variables(
    :private_key => "#{node['samplecompany-logger']['ssh_dir_path']}/#{node['samplecompany-logger']['ssh_key']}"
   )
end

...
file "#{node['samplecompany-reporttools']['ssh_dir_path']}/#{node['samplecompany-reporttools']['ssh_key']}" do
  content "#{node['samplecompany-reporttools']['ssh_key_content']}"
  mode '0600'
  owner 'root'
  group 'root'
end
...
execute "Retrieve public key from SSH private key" do
  cwd node['samplecompany-reporttools']['ssh_dir_path']
  command "ssh-keygen -y -f #{node['samplecompany-reporttools']['ssh_key']} > #{node['samplecompany-reporttools']['ssh_key']}.pub"
end

# Create the ssh wrapper to be used with GIT
template node['samplecompany-reporttools']['github_ssh_wrapper'] do
  ...
  group 'root'
  variables(
    :private_key => "#{node['samplecompany-reporttools']['ssh_dir_path']}/#{node['samplecompany-reporttools']['ssh_key']}"
   )
end

...
file "#{node['samplecompany-trackingbackend']['ssh_dir_path']}/#{node['samplecompany-trackingbackend']['ssh_key']}" do
  content "#{node['samplecompany-trackingbackend']['ssh_key_content']}"
  mode '0600'
  owner 'root'
  group 'root'
end
...
execute "Retrieve public key from SSH private key" do
  cwd node['samplecompany-trackingbackend']['ssh_dir_path']
  command "ssh-keygen -y -f #{node['samplecompany-trackingbackend']['ssh_key']} > #{node['samplecompany-trackingbackend']['ssh_key']}.pub"
end

# Create the ssh wrapper to be used with GIT
template node['samplecompany-trackingbackend']['github_ssh_wrapper'] do
  ...
  group 'root'
  variables(
    :private_key => "#{node['samplecompany-trackingbackend']['ssh_dir_path']}/#{node['samplecompany-trackingbackend']['ssh_key']}"
   )
end
